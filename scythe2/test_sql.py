...
test_data_2 = [{"Value":"A","variable":"alpha","value":2},
...
class TestSQL(unittest.TestCase):
    ...
    def test_join(self):
        ...
        print("---")
        print(q.stmt_string())
        #print(q.eval(inputs={0: test_data, 1: table_data_2}))

    def test_aggregate(self):
        q = Table(data_id=0)
        q1 = Aggregate(q, [1], -1, "mean")
        q2 = Table(data_id=1)
        q =  Join(q1, q2)
        
        inputs = {0: test_data, 1: table_data_2}

        print("---")
        print(q.stmt_string())
        print(q.is_abstract())
        schema, table_boundary = q.infer_output_info(inputs)
    ...
    def test_argmax(self):
        inputs = {0: test_data_2}

        q = Table(data_id=0)
        q1 = Aggregate(q, [1], 2, "max")
        q2 = Table(data_id=0)
        q = Join(q1, q2)
        q = Filter(q, [(0, "==", 3, "column"), (1, "==", 4, "column")])
        q = Project(q, (0, 1, 2))
        ...
        out_df = q.eval(inputs=inputs)
        obj = out_df.to_json(orient="records")
        print(obj)
