...
def pred_to_str(df, pred, hide_type=False):
	"""predicate to str"""
	def primitive_pred_to_str(p):
		pred_str = f"{df.columns[p[0]]} {p[1]} {p[2] if p[3] == 'value' else df.columns[p[2]]}"  
		if hide_type:
			return pred_str
		else:
			return pred_str + f"({p[4]})"
...
def check_pred(plist, primitive_pred_id_map):
	"""given a conjunctive predicate (represented as a list), 
	decide whether this is a valid predicate
	Here, it checks:
		* break symmetry (requires predicates to appear in an ascending order)
		* do not have > 2 predicates from the same category
		* each constant is only used once
	"""

	# break symmetry, this requires primitive predicates should have an ascending order in the conjunct
	for i in range(1, len(plist)):
		if primitive_pred_id_map[plist[i]] <= primitive_pred_id_map[plist[i-1]]:
			return False
...
def recursive_extract(tr, beam_size, primitive_pred_id_map):
	"""given the trace, and a map that labels id's of predicates, extract 
		up to *beam_size* number of predicates from the trace
	"""
	...
	for sub_tr in tr["left"]:
			...
			left += recursive_extract(sub_tr, beam_size, primitive_pred_id_map)
	...
	for plist in left:
		for p2 in right:
			...
			if check_pred(p, primitive_pred_id_map):
				conj_preds.append(p)
...
def instantiate_predicate(df, bv_by_size_w_trace, bv, beam_size=10):
	...
	# TODO: break symmetry
	primitive_pred_id_map = {}
	idx = 0
	for size in [1]:
		for temp_bv in bv_by_size_w_trace[size]:
			for pred in bv_by_size_w_trace[size][temp_bv]:
				primitive_pred_id_map[pred] = idx
				idx += 1
	...
	for size in bv_by_size_w_trace:
		if bv in bv_by_size_w_trace[size]:
			for tr in bv_by_size_w_trace[size][bv]:
				pred_list += recursive_extract(tr, beam_size, primitive_pred_id_map)
...
def enum_primitive_preds(df, dtypes, table_boundaries, constants):
	...
	primitives += [(0, "==", 0, "column", "empty_pred")]
...
if __name__ == '__main__':
	...
	q_list = q.replace_bvfilter_with_filter(inputs, ["A"], max_out_cnt=5)
